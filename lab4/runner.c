...
static int exec_mmap_and_check_result(page_table *pg_table, frame_entry const *frame_data, int const *page_dirty, int *page_mapped, pid_t child_pid) {
...
static int exec_munmap_and_check_result(page_table *pg_table, frame_entry *frame_data, int *page_dirty, int const *page_exists_on_disk, int pagenum, int *page_mapped, pid_t child_pid) {
...
static int ensure_page_loaded(page_table *pg_table, frame_entry const *frame_data, int const *page_dirty, int page_index, pid_t child_pid, int expects_segfault) {
            ...
            while (scanf(" %c", &instruction) == 1) {
                ...
                if (instruction == 'r' || instruction == 'w') {
                    if (!is_unmapped_page) {
                        ...
                        int res = ensure_page_loaded(pg_table, framedata, page_dirty, pagemap[pagenum], child_pid, is_unmapped_page);
                        if (res == -1) {
                            printf("Runner: Memory of user process is corrupted; OS has a bug!\n");
                            break;
                        }
                    ...
                    int const realnewpage = exec_mmap_and_check_result(pg_table, framedata, page_dirty, page_mapped, child_pid);
                    ...
                    int const res = exec_munmap_and_check_result(pg_table, framedata, page_dirty, page_exists_on_disk, pagemap[pagenum], page_mapped, child_pid);
                    if (res == -1) break;
                    if (!is_unmapped_page) {
